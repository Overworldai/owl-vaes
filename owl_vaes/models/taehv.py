...
class TAEHV(nn.Module):
    def __init__(self, checkpoint_path="taehv.pth", encoder_time_downscale=(True, True, False), decoder_time_upscale=(False, True, True), decoder_space_upscale=(True, True, True), patch_size=1, latent_channels=16, encoder_channels=64, decoder_channels=None, blocks_per_stage=3):
        ...
        Arg:
            ...
            encoder_channels: channel width for all encoder stages.
            decoder_channels: list of 4 channel widths for decoder stages. Defaults to [4*enc, 2*enc, enc, enc].
            blocks_per_stage: number of MemBlocks per encoder/decoder stage.
        """
        super().__init__()
        ...
        if decoder_channels is None:
            decoder_channels = [4 * e, 2 * e, e, e]
        n_f = list(decoder_channels)

        def _enc_stage(ch, tds):
            return [TPool(ch, 2 if tds else 1), conv(ch, ch, stride=2, bias=False)] + [MemBlock(ch, ch) for _ in range(blocks_per_stage)]

        self.encoder = nn.Sequential(
            conv(self.image_channels*self.patch_size**2, e), nn.ReLU(inplace=True),
            *_enc_stage(e, encoder_time_downscale[0]),
            *_enc_stage(e, encoder_time_downscale[1]),
            *_enc_stage(e, encoder_time_downscale[2]),
            conv(e, self.latent_channels),
        )

        def _dec_stage(ch_in, ch_out, space_up, time_up):
            return [MemBlock(ch_in, ch_in) for _ in range(blocks_per_stage)] + [
                nn.Upsample(scale_factor=2 if space_up else 1),
                TGrow(ch_in, 2 if time_up else 1),
                conv(ch_in, ch_out, bias=False),
            ]

        self.decoder = nn.Sequential(
            Clamp(), conv(self.latent_channels, n_f[0]), nn.ReLU(inplace=True),
            *_dec_stage(n_f[0], n_f[1], decoder_space_upscale[0], decoder_time_upscale[0]),
            *_dec_stage(n_f[1], n_f[2], decoder_space_upscale[1], decoder_time_upscale[1]),
            *_dec_stage(n_f[2], n_f[3], decoder_space_upscale[2], decoder_time_upscale[2]),
